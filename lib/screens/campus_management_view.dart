...
// ─── Local-only models (Stars & Posts have no API yet) ────────────────────────
...
class CampusPost {
  String id, authorName, authorPhotoUrl, caption, mediaUrl,
      thumbnailUrl, mediaType, clubTag, timeAgo;
  CampusPost({
    required this.id, required this.authorName, required this.authorPhotoUrl,
    required this.caption, required this.mediaUrl, required this.thumbnailUrl,
    required this.mediaType, required this.clubTag, required this.timeAgo,
  });
}

// ─── Helper: resolve current schoolId ────────────────────────────────────────
//  • correspondent  → sidebar selection (SchoolController.selectedSchool)
//  • all other roles → AuthController.user.schoolId
String _resolveSchoolId() {
  final auth = Get.find<AuthController>();
  final role = auth.user.value?.role?.toLowerCase() ?? '';
  if (role == 'correspondent') {
    return Get.find<SchoolController>().selectedSchool.value?.id ?? '';
  }
  return auth.user.value?.schoolId ?? '';
}
...
class _CampusManagementViewState extends State<CampusManagementView>
  ...
  late ClubsController _clubsCtrl;
  ...
  void initState() {
    ...
    _clubsCtrl = Get.find<ClubsController>();

    WidgetsBinding.instance.addPostFrameCallback((_) {
      final sid = _resolveSchoolId();
      if (sid.isNotEmpty) _clubsCtrl.loadClubs(sid);
    });
  }
        ...
        children: [
          ...
          Expanded(
            child: TabBarView(
              ...
              children: [
                // Tab 1 – Clubs (API-backed)
                const _ClubsTab(),
                // Tab 2 – Stars (local)
                _StarsTab(
                  stars: _stars,
                  clubs: _clubsCtrl.clubs.map((c) => c.name).toList(),
                  onAdd: (s) => setState(() => _stars.add(s)),
                  onEdit: (s) => setState(() {
                    final i = _stars.indexWhere((x) => x.id == s.id);
                    if (i >= 0) _stars[i] = s;
                  }),
                  onDelete: (id) =>
...
Widget _buildStyledInput(TextEditingController ctrl, String label, String hint,
...
Widget _buildUrlPreviewBox(IconData icon, String hint) {
  return Container(
    ...
    decoration: BoxDecoration(
      ...
      borderRadius: BorderRadius.circular(10),
      border: Border.all(color: Colors.grey.shade300, width: 0.5, style: BorderStyle.solid),
    ),
    child: Center(
      child: Row(
        ...
        children: [
          ...
          Text(hint, style: TextStyle(fontSize: 12, color: Colors.grey[400])),
        ],
      ),
    ),
  );
...
class _ClubsTabState extends State<_ClubsTab> {
  final ClubsController _ctrl = Get.find();
  ...
  Future<void> _submit() async {
    ...
    if (_editId != null) {
      // UPDATE
      final existing = _ctrl.clubs.firstWhereOrNull((c) => c.id == _editId);
      if (existing == null) return;
      await _ctrl.updateClub(Club(
        id: _editId!,
        name: name,
        ...
        classId: existing.classId, createdAt: '', updatedAt: '', coordinator: '', location: '',
      ));
    } else {
      // CREATE — banner URL not supported by API (API expects file upload).
      // Banner field is kept in the UI but not sent to the backend.
      await _ctrl.addClub(Club(
        id: '',
        name: name,
        description: _descCtrl.text.trim(),
        schoolId: schoolId,
        ...
        maxMembers: 100,
        category: 'General',
        classId: '', createdAt: '', updatedAt: '', coordinator: '', location: '',
      ));
    }
  ...
  Future<void> _confirmDelete(String clubId) async {
    ...
    if (ok == true) _ctrl.deleteClub(clubId);
  }

  void _startEdit(Club club) {
          ...
          children: [
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildStyledInput(_nameCtrl, 'Club name', 'e.g. Music, Dance, Theatre…'),
                    const SizedBox(height: 12),
                    _buildStyledInput(_descCtrl, 'Description',
                        'Brief description of the club…', maxLines: 3),
                    const SizedBox(height: 12),
                    _buildStyledInput(_bannerCtrl, 'Banner image URL',
                        'https://…/image.jpg', keyboardType: TextInputType.url),
                      ...
                      children: [
                        ...
                        ElevatedButton(
                          ...
                          child: isLoading
                              ...
                              : Text(_editId != null ? 'Update club' : 'Save club'),
                        ),
                      ],
                    ),
                  ],
            ...
            if (isLoading && clubs.isEmpty)
              ...
              const Center(
                child: Padding(
                  padding: EdgeInsets.all(32),
                  child: Text('No clubs yet. Add one!', style: TextStyle(color: Colors.grey)),
                ),
              )
            else
              Container(
                ...
                child: ClipRRect(
                  borderRadius: BorderRadius.circular(16),
                  child: Column(
                    children: clubs.map((c) => _ItemCard(
                      leading: Container(
                        width: 40, height: 40,
                        decoration: BoxDecoration(
                      ...
                      title: c.name,
                      subtitle: c.description.isNotEmpty ? c.description : 'No description',
                      onEdit: () => _startEdit(c),
...
// ─── CAMPUS STARS TAB — local state, UI unchanged ────────────────────────────
...
class _StarsTab extends StatefulWidget {
  ...
  const _StarsTab({
    required this.stars, required this.clubs,
...
class _StarsTabState extends State<_StarsTab> {
  ...
  @override
  void dispose() {
    _nameCtrl.dispose(); _roleCtrl.dispose();
    _photoCtrl.dispose(); _bioCtrl.dispose();
    super.dispose();
  }

  void _resetForm() {
    _nameCtrl.clear(); _roleCtrl.clear();
    _photoCtrl.clear(); _bioCtrl.clear();
    _selectedClub = ''; _editId = null;
  }
  ...
  void _submit() {
    ...
    final star = CampusStar(
      ...
      photoUrl: _photoCtrl.text.trim(), club: _selectedClub,
  ...
  void _startEdit(CampusStar s) {
    ...
    _photoCtrl.text = s.photoUrl; _bioCtrl.text = s.bio;
    _selectedClub = s.club; _editId = s.id;
    setState(() => _showForm = true);
  }
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, constraints) {
      ...
      return ListView(
        children: [
          _SectionHeader(
            title: 'Campus stars', addLabel: 'Add star',
            onAdd: () { _resetForm(); setState(() => _showForm = !_showForm); },
          ),
          if (_showForm) _buildForm(hPad: hPad),
          _buildList(hPad: hPad),
        ],
      );
    });
  }

  Widget _buildForm({double hPad = 16}) {
    return Container(
      margin: EdgeInsets.fromLTRB(hPad, 0, hPad, 12),
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          _buildStyledInput(_nameCtrl, 'Full name', 'e.g. Timothy White'),
          const SizedBox(height: 12),
          _buildStyledInput(_roleCtrl, 'Role / Title', 'e.g. Student leader'),
          const SizedBox(height: 12),
          _buildStyledInput(_photoCtrl, 'Profile photo URL', 'https://…/photo.jpg',
              keyboardType: TextInputType.url),
          const SizedBox(height: 8),
          _buildUrlPreviewBox(Icons.account_circle_outlined, 'Photo preview'),
          const SizedBox(height: 12),
          _buildStyledDropdown('Club / Category', _selectedClub, widget.clubs,
                  (v) => setState(() => _selectedClub = v ?? '')),
          const SizedBox(height: 12),
          _buildStyledInput(_bioCtrl, 'Bio (optional)', 'Short bio…', maxLines: 3),
          const SizedBox(height: 16),
          Row(
            mainAxisAlignment: MainAxisAlignment.end,
            children: [
  ...
  Widget _buildList({double hPad = 16}) {
    if (widget.stars.isEmpty) {
      return const Center(child: Padding(padding: EdgeInsets.all(32),
          child: Text('No campus stars yet. Add one!', style: TextStyle(color: Colors.grey))));
    }
    return Container(
      margin: EdgeInsets.symmetric(horizontal: hPad),
      decoration: BoxDecoration(
      ...
      child: ClipRRect(
        borderRadius: BorderRadius.circular(16),
        child: Column(
          children: widget.stars.map((s) => _ItemCard(
            leading: s.photoUrl.isNotEmpty
                ? CircleAvatar(radius: 20, backgroundImage: NetworkImage(s.photoUrl))
                : CircleAvatar(
...
// ─── POSTS TAB — local state, UI unchanged ───────────────────────────────────
...
class _PostsTab extends StatefulWidget {
  ...
  const _PostsTab({
    required this.posts, required this.clubs,
    required this.onAdd, required this.onEdit, required this.onDelete,
  });
...
class _PostsTabState extends State<_PostsTab> {
  ...
  @override
  void dispose() {
    _authorCtrl.dispose(); _authorPhotoCtrl.dispose(); _captionCtrl.dispose();
    _mediaCtrl.dispose(); _thumbCtrl.dispose();
    super.dispose();
  }

  void _resetForm() {
    _authorCtrl.clear(); _authorPhotoCtrl.clear(); _captionCtrl.clear();
    _mediaCtrl.clear(); _thumbCtrl.clear();
    _mediaType = 'video'; _selectedClub = ''; _editId = null;
  }
  ...
  void _startEdit(CampusPost p) {
    _authorCtrl.text = p.authorName; _authorPhotoCtrl.text = p.authorPhotoUrl;
    _captionCtrl.text = p.caption; _mediaCtrl.text = p.mediaUrl;
    _thumbCtrl.text = p.thumbnailUrl; _mediaType = p.mediaType;
    _selectedClub = p.clubTag; _editId = p.id;
    setState(() => _showForm = true);
  }
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, constraints) {
      ...
      return ListView(
        children: [
          _SectionHeader(
            title: 'Campus posts', addLabel: 'Add post',
            onAdd: () { _resetForm(); setState(() => _showForm = !_showForm); },
          ),
          if (_showForm) _buildForm(hPad: hPad),
          _buildList(hPad: hPad),
      ...
      child: Column(
        ...
        children: [
          Text(_editId != null ? 'Edit post' : 'New post',
              style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Color(0xFF1A1A2E))),
          const SizedBox(height: 12),
          _buildStyledInput(_authorCtrl, 'Author name', 'e.g. Cynthia Hall'),
          const SizedBox(height: 12),
          _buildStyledInput(_authorPhotoCtrl, 'Author photo URL', 'https://…/avatar.jpg',
              keyboardType: TextInputType.url),
          const SizedBox(height: 12),
          _buildStyledInput(_captionCtrl, 'Post caption / text',
              'Write something about the event…', maxLines: 3),
          const SizedBox(height: 12),
              ...
              Expanded(
                child: GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: _mediaType == 'image' ? Colors.blue[700] : Colors.white,
                      borderRadius: BorderRadius.circular(10),
                      border: Border.all(color: Colors.grey.shade300, width: 0.5),
                    ),
                    child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                      Icon(Icons.image_outlined, size: 16,
                          color: _mediaType == 'image' ? Colors.white : Colors.grey[600]),
              ...
              Expanded(
                child: GestureDetector(
                  ...
                  child: Container(
                    padding: const EdgeInsets.symmetric(vertical: 10),
                    decoration: BoxDecoration(
                      color: _mediaType == 'video' ? Colors.blue[700] : Colors.white,
                      borderRadius: BorderRadius.circular(10),
                      border: Border.all(color: Colors.grey.shade300, width: 0.5),
                    ),
                    child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                      Icon(Icons.videocam_outlined, size: 16,
                          color: _mediaType == 'video' ? Colors.white : Colors.grey[600]),
                      const SizedBox(width: 6),
          ...
          _buildStyledInput(
            ...
            _mediaType == 'video' ? 'https://…/video.mp4 or YouTube link' : 'https://…/image.jpg',
            keyboardType: TextInputType.url,
          ),
          const SizedBox(height: 12),
          _buildStyledInput(_thumbCtrl, 'Thumbnail image URL', 'https://…/thumb.jpg',
              keyboardType: TextInputType.url),
          const SizedBox(height: 8),
          _buildUrlPreviewBox(Icons.play_circle_outline, 'Thumbnail preview'),
          const SizedBox(height: 12),
          _buildStyledDropdown('Club tag', _selectedClub, widget.clubs,
                  (v) => setState(() => _selectedClub = v ?? '')),
          const SizedBox(height: 16),
          Row(
            mainAxisAlignment: MainAxisAlignment.end,
            children: [
              ...
              ElevatedButton(
                ...
                child: Text(_editId != null ? 'Update post' : 'Publish post'),
              ),
            ],
          ),
        ],
  ...
  Widget _buildList({double hPad = 16}) {
    if (widget.posts.isEmpty) {
      return const Center(child: Padding(padding: EdgeInsets.all(32),
          child: Text('No posts yet. Add one!', style: TextStyle(color: Colors.grey))));
    }
    return Column(
      children: widget.posts.map((p) => Container(
        margin: EdgeInsets.fromLTRB(hPad, 0, hPad, 12),
        decoration: BoxDecoration(
          color: Colors.white, borderRadius: BorderRadius.circular(16),
          boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.05), blurRadius: 10, offset: const Offset(0, 2))],
              ...
              child: Row(
                children: [
                  p.authorPhotoUrl.isNotEmpty
                      ? CircleAvatar(radius: 18, backgroundImage: NetworkImage(p.authorPhotoUrl))
                      : CircleAvatar(
                      radius: 18, backgroundColor: Colors.pink.shade100,
            ...
            if (p.thumbnailUrl.isNotEmpty)
              ClipRRect(
                ...
                child: Stack(
                  ...
                  children: [
                    Image.network(p.thumbnailUrl, width: double.infinity, height: 160, fit: BoxFit.cover,
